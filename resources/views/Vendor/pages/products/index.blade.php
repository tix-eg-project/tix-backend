...
<div class="content-wrapper">
  <div class="container-xxl flex-grow-1 container-p-y">
    ...
    <div class="card mb-3">
      <div class="card-body">
        <form method="GET" action="{{ route('vendor.products.index') }}" id="searchForm">
          <div class="row g-2 align-items-end">
            ...
            <div class="col-12 col-md-4">
              ...
              <div class="input-group">
                ...
                @if(request('search'))
                <a href="{{ route('vendor.products.index', collect(request()->except(['search','page']))->filter()->all()) }}"
            ...
            {{-- Subcategory (يتأثر بالـ category لو مبعوت من الكنترولر) --}}
            <div class="col-6 col-md-2">
    ...
    <div class="card">
        ...
        <table class="table table-hover table-striped text-center align-middle mb-0">
          <thead class="table-light">
            <tr>
              ...
              <th class="text-end" style="width:260px">{{ __('messages.Actions') }}</th>
            </tr>
          </thead>
          <tbody>
            ...
            <tr>
              ...
              <td class="fw-semibold">{{ $product->name_text ?? '—' }}</td>
              <td class="text-muted">{{ \Illuminate\Support\Str::limit($product->short_description_text ?? '', 80) ?: '—' }}</td>
              ...
              <td>
                <div class="text-warning">
                  {{ $product->average_rating }} <i class="bx bxs-star"></i>
                </div>
              </td>
              <td>
              ...
              <td class="text-end">
                <a href="{{ route('vendor.products-variant.create', $product) }}" class="btn btn-sm btn-info">
                ...
                @endphp
                <a href="{{ route('vendor.products-variant.index', $product) }}" class="btn btn-sm btn-outline-info position-relative" title="{{ __('messages.Sub Products') }}">
                ...
                <a href="{{ route('vendor.reviews.index', ['product_id' => $product->id]) }}" class="btn btn-sm btn-outline-warning" title="{{ __('messages.Reviews') }}">
                  <i class="bx bx-star"></i>
                </a>

                <button type="button" class="btn btn-sm btn-outline-success btn-view-features"
                        title="{{ __('messages.Features') ?? 'Features' }}"
                        data-name="{{ addslashes($product->name_text) }}"
                ...
                <form action="{{ route('vendor.products.delete', $product->id) }}" method="POST" class="d-inline">
                  ...
                  <button type="submit" class="btn btn-sm btn-danger"
                    onclick="return confirm('{{ __('messages.are_you_sure') ?? 'Are you sure?' }}')">
            ...
            @empty
            <tr>
              <td colspan="12" class="text-center text-muted py-4">{{ __('messages.no_products') ?? 'No products found' }}</td>
            </tr>
            @endforelse
          </tbody>
        </table>
      ...
      <!-- <a href="{{ route('vendor.dashboard') }}" class="btn btn-secondary m-3">{{ __('messages.Back') }}</a> -->
    </div>
...
<script>
  ...
  function showFeaturesModal(productName, features) {
      ...
      if (features && features.length > 0) {
          ...
          features.forEach(f => {
              if (f && f.trim() !== '') {
                  html += `<li class="list-group-item d-flex align-items-start border-0 ps-0">
