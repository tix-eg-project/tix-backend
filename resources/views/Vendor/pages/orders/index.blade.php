...
<div class="content-wrapper">
    <div class="container-xxl flex-grow-1 container-p-y"><!-- ✅ يضيف padding علوي وجانبي -->
        ...
        @php
        $statusOptions = ($statusOptions ?? ['placed','paid','shipped','delivered','canceled','pending']);
        $paymentOptions = ($paymentOptions ?? collect($orders->items() ?? [])->pluck('payment_method_name')->filter()->unique()->values()->all());
        $badgeMap = [
        ...
        <div class="card mb-3">
            <div class="card-body">
                <form method="GET" action="{{ route('vendor.orders.index') }}" id="searchForm">
                ...
                <div class="table-responsive">
                    <table class="table table-striped table-hover align-middle mb-0">
                        ...
                        <tbody>
                            ...
                            @php $stClass = $badgeMap[$order->status] ?? 'bg-secondary-subtle text-secondary border border-secondary-subtle'; @endphp
                            <tr>
                                <td>{{ $order->id }}</td>
                                ...
                                <td>{{ number_format((float)($order->vendor_total ?? $order->total), 2) }} {{ __('messages.currency') }}</td>
                                <td class="text-end">
                                    <a href="{{ route('vendor.orders.edit', $order->id) }}" class="btn btn-sm btn-primary" title="{{ __('messages.edit') }}">
                                        <i class="fa-regular fa-pen-to-square"></i>
                                    </a>
                                    <a href="{{ route('vendor.orders.show', $order->id) }}" class="btn btn-sm btn-info" title="{{ __('messages.details') }}">
                                        <i class="fa-regular fa-eye"></i>
                                    </a>
                                    <form method="POST" action="{{ route('vendor.orders.destroy', $order->id) }}" class="d-inline-block" onsubmit="return confirm('{{ __('messages.are_you_sure') }}')">
                                        @csrf @method('DELETE')
