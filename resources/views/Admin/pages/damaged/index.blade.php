...
@php
    ...
    // يحوّل أي مدخل لصورة إلى URL صالح
    $normalizeImg = function ($src) {
        if (!$src) return null;
        $src = trim((string)$src);

        if (Str::startsWith($src, ['http://','https://','//','data:image'])) return $src; // URL جاهز
        if (Str::startsWith($src, ['/'])) return $src; // مطلق على نفس الدومين
        ...
        // محاولة أخيرة: اعتباره نسبي
        return url($src);
    };

    // يلمّ كل المرشحين المحتملين للصورة: من سطر الأوردر (عبر returnRequest) ثم المنتج
    $pickImageForDamaged = function ($dam) use ($normalizeImg) {
        ...
        $candidates = [
            // من سطر الأوردر (لو الطلب التالف مرتبط بطلب استرجاع)
            $oi?->image_url ?? null,
            $oi?->product_image ?? null,
            $oi?->image ?? null,
            $oi?->thumbnail ?? null,

            // من الڤاريانت (لو عندك صورة على الڤاريانت)
...
<div class="card rounded-4 border-0 shadow-sm mb-3">
    <div class="card-body">
        <form method="GET" action="{{ route('admin.damaged-stocks.index') }}">
            <div class="row g-3 align-items-end">
                ...
                <div class="col-12 col-md-2">
                    <a href="{{ route('admin.damaged-stocks.index') }}" class="btn btn-outline-secondary w-100">
                        {{ __('messages.clear') }}
                    </a>
                </div>
                @endif
                ...
                <tbody>
                    @forelse($items as $it)
                        ...
                        <tr>
                            ...
                            <td>
                                <a href="{{ route('admin.damaged-stocks.show', $it->id) }}"
