...
@section('title', __('messages.Update Brand'))

@section('content')
<div class="card">
    <div class="card-body">
        <h5 class="card-title text-center">{{ __('messages.Update Brand') }}</h5>
        ...
        <form method="POST" action="{{ route('brands.update', $brand->id) }}" enctype="multipart/form-data">
            ...
            <div class="mb-3">
                ...
                <input type="text" name="name[{{ $localeCode }}]"
                    ...
                    value="{{ old("name.$localeCode") ?? (json_decode($brand->getRawOriginal('name'), true)[$localeCode] ?? '') }}">
