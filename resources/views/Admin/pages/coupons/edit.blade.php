...
<div class="card-clean p-4">
    <h5 class="mb-3">{{ __('messages.edit_coupon') }}</h5>
        ...
        <div class="row g-3">
            ...
            <div class="col-md-4">
                ...
                <select name="discount_type" class="form-select @error('discount_type') is-invalid @enderror">
                    <option value="percent" @selected(old('discount_type',$coupon->discount_type)=='percent')>{{ __('messages.percent') }}</option>
                    <option value="amount" @selected(old('discount_type',$coupon->discount_type)=='amount')>{{ __('messages.amount') }}</option>
                </select>
            ...
            <div class="col-md-4">
                ...
                <input type="number" step="0.01" name="discount_value" class="form-control @error('discount_value') is-invalid @enderror" value="{{ old('discount_value', $coupon->discount_value) }}" required>
            ...
            <div class="col-md-4">
                ...
                <input type="number" min="0" name="max_uses" class="form-control @error('max_uses') is-invalid @enderror" value="{{ old('max_uses', $coupon->max_uses) }}" placeholder="∞">
                @error('max_uses') <div class="invalid-feedback d-block">{{ $message }}</div> @enderror
            </div>

            <div class="col-md-4">
                <label class="form-label">{{ __('messages.starts_at') }}</label>
                <input type="date" name="starts_at" class="form-control @error('starts_at') is-invalid @enderror" value="{{ old('starts_at', optional($coupon->starts_at)->format('Y-m-d')) }}" required>
            ...
            <div class="col-md-4">
                ...
                <input type="date" name="ends_at" class="form-control @error('ends_at') is-invalid @enderror" value="{{ old('ends_at', optional($coupon->ends_at)->format('Y-m-d')) }}" required>
