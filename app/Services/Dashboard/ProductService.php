...
class ProductService
{
    ...
    public function store(array $data): Product
    {
        ...
        // حفظ المميزات
       if (!empty($data['prod_features'])) {
            foreach ($data['prod_features'] as $featureData) {
                if (!empty($featureData['ar']) || !empty($featureData['en'])) {
                    $product->prod_features()->create(['name' => $featureData]);
                }
            }
        }
        ...
        if (!empty($data['qas'])) {
            foreach ($data['qas'] as $qaData) {
                $hasQuestion = !empty($qaData['question_ar']) || !empty($qaData['question_en']);
                $hasAnswer = !empty($qaData['answer_ar']) || !empty($qaData['answer_en']);

                if ($hasQuestion || $hasAnswer) {
                    $product->qas()->create([
                        'question' => [
                            'ar' => $qaData['question_ar'] ?? '',
                            'en' => $qaData['question_en'] ?? '',
                        ],
                        'answer' => [
                            'ar' => $qaData['answer_ar'] ?? '',
                            'en' => $qaData['answer_en'] ?? '',
    ...
    public function update(Product $product, array $data)
    {
        ...
        if (array_key_exists('faqs', $data)) {
         ...
         // تحديث المميزات (حذف وإعادة إضافة)
        // نتحقق من وجود 'features' أو وجود 'features_loaded' للتأكد من أن القسم تم إرساله
        if (isset($data['prod_features']) || isset($data['prod_features_loaded'])) {
            $product->prod_features()->delete();
            if (!empty($data['prod_features'])) {
                foreach ($data['prod_features'] as $featureData) {
                    if (!empty($featureData['ar']) || !empty($featureData['en'])) {
                        $product->prod_features()->create(['name' => $featureData]);
        ...
        // تحديث الأسئلة والأجوبة (حذف وإعادة إضافة)
        if (isset($data['qas']) || isset($data['qas_loaded'])) {
            $product->qas()->delete();
            if (!empty($data['qas'])) {
                foreach ($data['qas'] as $qaData) {
                    $hasQuestion = !empty($qaData['question_ar']) || !empty($qaData['question_en']);
